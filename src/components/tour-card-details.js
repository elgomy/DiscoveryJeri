...
class TourCardDetails extends LitElement {
  render() {
    return html`
       ${SharedStyles}
      	<style>
          ...
          <div class="time__container">
            <p class="time"><iron-icon icon="vaadin:clock"></iron-icon> Duração: ${this.time} <img src="${this.icon}" alt="vehicle" /></p>
    			...
    			<div class="card-actions">
      			<paper-button @click="${this.requestButton}">Solicitar</paper-button>
            <paper-button @click="${this.detailsButton}">Detalhes</paper-button>
  ...
  detailsButton(){
    this.dispatchEvent(new CustomEvent('openDetails',{bubbles: true, composed: true, detail:this.header}));
  }
}

window.customElements.define('tour-card-details', TourCardDetails);